...
- [ ] Community page + Home "Active Traders": full trader directory (founding-member highlight, per-trader cards with "trading now" status, name search, city filter, sort) needs a traders-directory service (list individual traders with stats). For now `/community` shows real community stats + an active-cities directory from `getHeroCities`; the TraderBadge primitive is ready for the per-trader cards once the service exists.
- [ ] Admin fidelity polish: the admin dashboard + subpages (reports/feedback/users/trades/growth) were retheme-swept to the palette (structural neutrals + status-color families mapped to terra/teal/brass/slate). Status pills and stat-card accents are coherent but were mapped programmatically, not hand-tuned to the Admin Subpages mock — give them a visual pass against the mock when convenient. Admin pages are auth+is_admin gated so they weren't visually verified during the reskin.
- [ ] Market Pulse (`/pulse`): mock has demand-vs-supply ratios (want-lists vs listings per commander), a Most Viewed panel (needs view tracking), and a This week / 30-day toggle. Built from real signals available now (deck interest = "most wanted", city activity = "local scene"); the diverging demand/supply bar, most-viewed, and time-window toggle need new aggregate/view-tracking services.
- [ ] Profile earnable Badges section (Deck Listing / Trades / Want Lists / Likes Received, earned vs locked + progress) — needs a badges/achievements system (data layer). Omitted from the profile reskin.
