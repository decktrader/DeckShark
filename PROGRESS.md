...
**Milestone:** M16 Security Hardening — PR open
**Status:** PR #13 open on `m16-security-hardening`. All security items committed + commander name field on deck creation form.
**Next step:** Merge PR #13, run migration 020 on production Supabase, then start M17 (Rate Limiting).
...
| **M16: Security Hardening**    | PR Open  | PR #13. RLS column restrictions, security headers, account deletion confirmation, UUID validation, auth error generification, commander name field. |
...
### 2026-04-04 — M16 PR opened + commander field + Phase 3 concept

**Done:** Committed all M16 security hardening work and opened PR #13. Added commander name input field to deck creation form (explicit field takes priority over COMMANDER: lines in decklist). Added Phase 3 deck rotation subscription concept to PLAN.md.
**Next:** Merge PR #13, run migration 020 on production Supabase. Then start M17 (Rate Limiting & Abuse Prevention).

### 2026-04-03 — M16 Security Hardening (implementation)
