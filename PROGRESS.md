...
**Milestone:** Visual Redesign — "The Convention Floor / The Harbour"
**Status:** Full presentation-layer reskin from the `design-redesign/` handoff is COMPLETE across every page. Dark "Dark Arcane" theme replaced with the warm-paper / midnight-navy palette (terracotta + teal semantic pair, brass accent). 16 commits on `main` (after the `pre-redesign-snapshot` baseline), nothing pushed. `pnpm build` passes clean (exit 0, 48 pages). Only the presentation layer changed — no routing/data/Supabase/server-logic edits. Two new routes added: `/community` and `/pulse`.
**What shipped:** design tokens + next/font (Space Grotesk / Figtree / Spline Sans Mono); shared chrome (TopNav, Footer) + ds/ primitives (Brand, Pfp, Tag, NavyPanel, DeckCard, TraderBadge); and every page: Browse, Deck Detail, Home (4 hero states), Dashboard, Trades (list/detail/new), Want Lists (dir/detail/create), Profile, Community (new), New/Edit Deck, Notifications, Settings, Onboarding, Auth, About/Privacy, Market Pulse (new), Admin. Old map-based hero + DeckBrowseCard deleted as dead code.
**Next step:** Review the redesign in the browser (esp. signed-in/admin pages, which are auth-gated and weren't visually verified). Then decide on PUSH (16 commits sit unpushed on `main`; pushing triggers a Vercel prod deploy — confirm first). Then work the redesign follow-ups in BACKLOG.md (2026-06-29 section): want-list aggregate + filters, Watch/favourite feature, Active Traders directory, Profile badges, Market Pulse demand/supply + toggle, admin fidelity polish, list-view. Marketing M1 (demo video, trade screenshot, how-it-works graphic) still pending and was paused for the redesign.
**Blocked:** Demo video + trade screenshot blocked on having a completed trade. Several redesign sections (aggregates, trader directory, badges, view tracking) are blocked on net-new data services — all backlogged, intentionally out of scope for a presentation-only reskin.
...
| **Visual Redesign (Convention Floor)**  | Complete    | Full presentation-layer reskin to warm-paper/navy theme. Tokens+fonts, shared chrome + ds/ primitives, every page restyled, /community + /pulse added. 16 commits on main, unpushed. `pnpm build` clean. Data-dependent sections backlogged (2026-06-29). |
...
### 2026-06-29 — Visual redesign: "The Convention Floor" (full reskin)

Full presentation-layer redesign from the `design-redesign/` handoff. Replaced the dark "Dark Arcane" theme with the warm-paper / midnight-navy palette (terracotta = wanted/hot, teal = listed/go, brass = featured), Space Grotesk / Figtree / Spline Sans Mono fonts. No routing/data/Supabase/server-logic changes.

- **Tokens + fonts:** ported `deckshark.css` `:root` into `globals.css`, repointed shadcn semantic tokens, exposed DeckShark utilities via `@theme`; `next/font/google`; dropped the `dark` class.
- **Chrome + primitives:** rebuilt TopNav + Footer; new `src/components/ds/` (Brand, Pfp, Tag/NavLight, NavyPanel + DivergingBar, DeckCard, TraderBadge); extended Button (terra/brass/navy + press-shadow); ColorPips ring treatment.
- **Pages:** Browse, Deck Detail, Home (4 hero states + live feed + ticker), Dashboard, Trades (list/detail/new), Want Lists (dir/detail/create+edit), Profile, **Community (new `/community`)**, New/Edit Deck, Notifications, Settings, Onboarding, Auth (navy split), About/Privacy, **Market Pulse (new `/pulse`)**, Admin (+ subpages, programmatic palette sweep).
- **Cleanup:** deleted orphaned map-based hero (`components/hero/*`) and `deck-browse-card.tsx`.
- **Verified:** `pnpm build` exit 0, 48 pages, zero errors. 16 commits on `main`, **not pushed** (push triggers prod deploy — awaiting confirmation). Snapshot/rollback: tag `pre-redesign-2026-06-29` + branch `pre-redesign-snapshot` (pushed).
- **Deferred (need net-new data services, see BACKLOG 2026-06-29):** want-list most-wanted aggregate + filter/sort toolbar; separate Watch/favourite feature (distinct from the shipping-vote `deck_interests`); Active Traders directory; Profile earnable badges; Market Pulse demand/supply ratio + Most Viewed + week toggle; admin fidelity polish; Browse true list-view; clickable mana-pip color filter.

**Next:** browser review (signed-in + admin pages weren't visually verified — auth-gated); then confirm push; then work the backlog items.
